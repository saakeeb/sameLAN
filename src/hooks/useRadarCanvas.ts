...
/** Minimal peer shape needed by the radar canvas. */
export interface RadarPeer {
  id: string;
  status: 'connecting' | 'connected';
}

/** Current mode chip shown in the lower-right corner. */
export type ActiveMode = 'file' | 'chat' | 'voice' | null;

interface UseRadarCanvasOptions {
  /** A ref to the <canvas> the radar should draw into. */
  canvasRef: RefObject<HTMLCanvasElement | null>;
  /** All peers we should orbit around the center. */
  peers: RadarPeer[];
  /** Local peer id; the first 6 chars render in the center node. */
  peerId: string | null;
  /** Optional mode chip label. */
...
/**
 * Drives the animated network radar. Owns the requestAnimationFrame loop, the
 * DPR-aware resize handler, and all the ctx.draw* calls. The component layer
 * is responsible only for mounting the <canvas> and forwarding props.
 */
export const useRadarCanvas = ({
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const angleStep =
        connectedPeers.length > 0 ? (Math.PI * 2) / connectedPeers.length : 0;
      ...
      ctx.textAlign = 'left';
      ctx.fillText(
        `${connectedPeers.length} NODE${connectedPeers.length !== 1 ? 'S' : ''} ONLINE`,
        8,
        H - 8,
      );
  ...
  }, [canvasRef, peers, peerId, activeMode]);
};

export default useRadarCanvas;
